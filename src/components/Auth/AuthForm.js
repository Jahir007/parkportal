...
const AuthForm = () => {
  ...
  const submitHandler = async (event) => {
    ...
    if ((enteredPassword.includes(' ') || enteredPassword.length < 6) && !isLogin) {
  ...
  return (
    <section className={classes.auth}>
      ...
      <form onSubmit = {submitHandler}>
        ...
        <div >
          {!isLoading && <button className='btn-outline-primary btn-lg mb-2'>{isLogin ? 'Login' : 'Create Account'}</button>}
          {isLoading && <Loader/>}
        </div>
      </form>
    </section>
